...
func checkSum(sheet: String) -> Int {
    let stringArrs = sheet.split(separator: "\n")
    var numArr = [[Int]]()
    
    for arr in stringArrs {
        let intArr = arr.split(separator: " ").flatMap { Int(String($0)) }
        numArr.append(intArr)
    }
    
    return checkSum(sheet: numArr)
}
...
func checkSum(sheet: [[Int]]) -> Int {
    ...
    for row in sheet {
        let sortedRow = row.sorted { $0 > $1 }
        let runs = row.count - 1
        for index in 0..<runs {
            let newRun = runs - index
            var index2 = index + 1
            
            for _ in 0..<newRun {
                let num1 = sortedRow[index]
                let num2 = sortedRow[index2]

                if num1 % num2 == 0 {
                    let diff = num1 / num2
                    reducedArr.append(diff)
                }
                index2 = index2 + 1
            }
        }
    ...
    let sum = reducedArr.reduce(0, +)
    return sum
}

print(checkSum(sheet: string))
