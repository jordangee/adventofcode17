let str1 = "abcde fghij"
let str2 = "abcde xyz ecdab"
let str3 = "a ab abc abd abf abj"
let str4 = "iiii oiii ooii oooi oooo"
let str5 = "oiii ioii iioi iiio"
let multiline = """
abcde fghij
abcde xyz ecdab
...
func isValid(_ string: String) -> Bool {
    let wordArr = string.split(separator: " ")
    let runs = wordArr.count - 1
    
    for index in 0..<runs {
        var index2 = index + 1
        for _ in index..<runs {
            let firstWord = wordArr[index]
            let secondWord = wordArr[index2]
            if firstWord == secondWord {
                return false
            }
            
            if firstWord.count == secondWord.count {
                let sortedWord1 = firstWord.characters.sorted { $0 < $1 }
                let sortedWord2 = secondWord.characters.sorted { $0 < $1 }
...
func getValidCount(_ multiline: String) -> Int {
    let passphraseArr = multiline.split(separator: "\n")
    var validCount = 0
    for passphrase in passphraseArr {
        if isValid(String(passphrase)) {
            validCount += 1
        }
    }
    return validCount
